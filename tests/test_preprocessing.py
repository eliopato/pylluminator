...
def test_infer_infinium_I_channel(test_samples):
    ...
    assert summary.to_numpy().tolist() == [44984, 701, 52, 82558]
...
def test_infer_infinium_I_channel_switch(test_samples):
    ...
    assert summary.to_numpy().tolist() == [44803, 882, 67, 82543]
...
def test_dye_bias_corr(test_samples):
    ...
    assert test_samples.get_probes('cg00002033_TC12')['PREC_500_3'].to_numpy()[0].tolist() == pytest.approx(expected_values)
    # Type I red
    expected_values = [213.75865173339844, 205.05917358398438, 1456.2840576171875, 1399.2308349609375]
    assert test_samples.get_probes('rs6991394_BC11')['PREC_500_3'].to_numpy()[0].tolist() == pytest.approx(expected_values)
