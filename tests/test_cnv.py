...
from pylluminator.cnv import copy_number_segmentation, get_normalization_samples
...
def test_cns_wrong_sample_name(test_samples):
    assert copy_number_segmentation(test_samples, 'wrongname') == (None, None, None)

def test_cns_wrong_normalization_sample_name(test_samples):
    assert copy_number_segmentation(test_samples, 'PREC_500_3', 'wrongname') == (None, None, None)

def test_cns_non_existent_normalization(test_samples):
    test_samples.annotation = Annotations(ArrayType.HUMAN_MSA, GenomeVersion.HG38)
    assert copy_number_segmentation(test_samples, 'PREC_500_3') == (None, None, None)